...
void HttpPerRequestTapperImpl::onRequestTrailers(const Http::HeaderMap& headers) {
  config_->rootMatcher().onHttpRequestTrailers(headers, statuses_);
}
...
void HttpPerRequestTapperImpl::onResponseTrailers(const Http::HeaderMap& headers) {
  config_->rootMatcher().onHttpResponseTrailers(headers, statuses_);
}
